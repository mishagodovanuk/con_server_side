...
$("#document-preview-button2").click(function () {
    $(document).ready(function () {
        let table = $("#container-table-preview");
        let table2 = $("#container-table-preview2");


        let isRowHovered = false;

        const mainObject = JSON.parse(localStorage.getItem("settings"));
        const container = mainObject.fields.container;
        // console.log(nomenclature);
        const datafields = Object.entries(container).map(([key, value]) => {
            return {
                name: key,
                type: "number",
        ...
        const columns = Object.entries(container).map(([key, value]) => {
            return {
                dataField: key,
