...
$("#document-preview-button2").click(function () {
    $(document).ready(function () {
        let table = $("#service-table-preview");
        let table2 = $("#service-table-preview2");


        let isRowHovered = false;

        const mainObject = JSON.parse(localStorage.getItem("settings"));
        const services = mainObject.fields.services;
        // console.log(nomenclature);
        const datafields = Object.entries(services).map(([key, value]) => {
            return {
                name: key,
                type: "number",
        ...
        const columns = Object.entries(services).map(([key, value]) => {
            console.log(value)
            return {
                dataField: key,
