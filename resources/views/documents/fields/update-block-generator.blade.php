    ...
    @switch($field['type'])
        @case('text')
            <input
                ...
                value="{{$document->data()['header'][$key]}}"
            ...
            <div class="row">
                <div class="col-6">
                    <input type="number"
                           ...
                           name="{{$key}}[]"
                           id="{{$key}}"
                           value="{{$document->data()['header'][$key][0]}}"
                        {{$field['required'] ? 'required' : ''}}>
                </div>
                ...
                <div class="col-6">
                    <input type="number"
                           ...
                           name="{{$key}}[]"
                           id="{{$key}}"
                           value="{{$document->data()['header'][$key][1]}}"
                        {{$field['required'] ? 'required' : ''}}>
                </div>
            </div>
            @break
        ...
        @case('dateRange')
            <input type="text"
                   ...
                   placeholder="{{$field['hint']}}"
                   readonly="readonly"
                   value="{{$document->data()['header'][$key][0]}} to {{$document->data()['header'][$key][1]}}"
                {{$field['required'] ? 'required' : ''}}
            >
            @break
        @case('dateTimeRange')
            <div class="row">
                ...
                <div class="col-3">
                    <input type="text"
                           id="{{$key}}_time_to"
                           value="{{$document->data()['header'][$key][2]}}"
                           name="{{$key}}[]"
                           class="form-control flatpickr-time text-start flatpickr-input {{$field['required'] ? 'required-field' : ''}}"
                           placeholder="До"
                           readonly="readonly"
        ...
        @case('select')
            <select
                ...
                data-dictionary="{{$documentType->settings()['fields']['header'][$key]['directory']}}">
                {{$field['required'] ? 'required' : ''}}
            </select>
            @break
        @case('label')
            <select class="select2 form-select select2-multiple dictionary-search
                    ...
                    id="{{$key}}"
                    data-id="{{json_encode($document->data()['header'][$key])}}"
                    data-dictionary="{{$documentType->settings()['fields']['header'][$key]['directory']}}"
                    name="{{$key}}[]"
                    data-placeholder="{{$field['hint']}}"
                    {{$field['required'] ? 'required' : ''}}
        ...
        @case('switch')
            <div
                ...
                <input type="checkbox"
                       {{$document->data()['header'][$key] == "on" ? 'checked' : ''}}
