...
let typeBlockNameUpdate
let typeServiceSideUpdate
...
function firstDownloadPage() {
    $("#initialConditions").removeClass("d-none");
    checkContractSigned();
}
...
$(document).on(
    ...
    function (e) {
        ...
        if (!$(this)[0].hasAttribute('data-bs-toggle')) {
            let formData = getContractData();
            formData.append('company_regulation_id', getSelectedRegulation());

            fetch(window.location.origin + '/contracts', {
                method: 'POST',
                body: formData
