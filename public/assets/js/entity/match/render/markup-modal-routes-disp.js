...
export function markupModalRoutesDisp(route) {
    const url = window.location.origin;
    if(route[0].type === "upload") {
        $('#total-members-modal').hide()
    } else {
        $('#total-members-modal').show();
    }
    return route.map(
        (
            {
    ...
    <div class="col-6" style="display: flex; flex-direction: column; gap: 8px;">
        ...
        <div class="d-flex" style="gap: 10px;">
            <div class="d-flex align-items-center" id="total-members-modal" style="background: rgba(168, 170, 174, 0.16); padding: 2px 10px; border-radius: 4px; gap:5px;">
            ...
            <div class="d-flex align-items-center" style="background: rgba(168, 170, 174, 0.16); padding: 2px 10px; border-radius: 4px; gap:8px;">
                <img src="${url}/assets/icons/box-multiple.svg"/> <p class="match-light-gray-text m-0 ">${current_pallets}/${common_pallets}</p>
            </div>
